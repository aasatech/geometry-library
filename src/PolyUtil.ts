...
class PolyUtil {
  ...
  static isLocationOnEdgeOrPath(point: Path, poly: Path[], closed: boolean, geodesic: any, toleranceEarth: number) {
    ...
    if (geodesic) {
      ...
      for (let i in poly) {
        ...
        let lng2 = deg2rad(val["lng"]);
        if (
          max(lat1, lat2) >= minAcceptable &&
          min(lat1, lat2) <= maxAcceptable
        ) {
          // We offset longitudes by -lng1; the implicit x1 is 0.
          let x2 = MathUtil.wrap(lng2 - lng1, -Math.PI, Math.PI);
          let x3Base = MathUtil.wrap(lng3 - lng1, -Math.PI, Math.PI);
          xTry[0] = x3Base;
  ...
  static sinDeltaBearing(lat1: number, lng1: number, lat2: number, lng2: number, lat3: number, lng3: number): number {
    ...
    const b = sin(lat31) + 2 * sinLat1 * cosLat3 * MathUtil.hav(lng31);
    const d = sin(lat21) + 2 * sinLat1 * cosLat2 * MathUtil.hav(lng21);
  ...
  static distanceToLine(p: Path, start: Path, end: Path) {
    ...
    const s0lat = deg2rad(p["lat"]);
    const s0lng = deg2rad(p["lng"]);
    const s1lat = deg2rad(start["lat"]);
    const s1lng = deg2rad(start["lng"]);
    const s2lat = deg2rad(end["lat"]);
    const s2lng = deg2rad(end["lng"]);
